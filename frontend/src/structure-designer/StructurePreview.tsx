 ...
 *
 * It owns the store subscriptions and hands the whole operation list and the
 * current layer down, so a preview is a plain function of what it is given —
 * which is also what keeps preview modules out of the store's import graph. It
 * lives inside the <Canvas>, so its subscriptions are what drive the repaint.
 */
export default function StructurePreview() {
  ...
  return <Preview operation={operation} operations={operations} layer={layer} />;
}