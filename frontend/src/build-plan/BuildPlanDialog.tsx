...
/**
 * The options dialog for a printed build plan, and the one place a plan is
...
function Dialog({ request }: { request: BuildPlanRequest }) {
  ...
  // Nothing to print: the element was deleted, or has no dominoes. The menu item
  // is gated on the same thing, so this only shows if something changed while
  // the dialog was already up.
  if (!model) {
    return (
      <ConfirmDialog
        message="This element has no dominoes to print."
        cancelLabel="Close"
  ...
  const print = () => {
    ...
    // it must be, or the browser stops seeing the click that led here and its
    // pop-up blocker refuses the new tab.
    if (!openPlanTab(definition.render(model, options))) {
    ...
    <>
      ...
      >
        ...
        <div className={styles.footer}>
          ...
          <button className={`${styles.button} ${styles.primary}`} onClick={print}>
            Output for Print
          </button>
        </div>
      </div>
